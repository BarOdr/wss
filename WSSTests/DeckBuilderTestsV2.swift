...
final class DeckBuilderTestsV2: XCTestCase {
    ...
    func test_deckBuilder_easy_no_addons() {
        ...
        XCTAssertEqual(getBaseGeneralCards(from: decks.actionDeck).count, 1)
    ...
    private func getLegendaryHuntCards(from array: [ActionCardModel]) -> [ActionCardModel] {
        array.filter { card in
            card.cardType == .legendaryHunt
        }
    }

    private func getLevel1Cards(from array: [ActionCardModel]) -> [ActionCardModel] {
        array.filter { card in
            card.level == 1
        }
    }

    private func getLevel2Cards(from array: [ActionCardModel]) -> [ActionCardModel] {
        array.filter { card in
            card.level == 2
        }
    }
